...
# LWF_VR测试脚本 - 11x11矩阵评估
# 测试每个LWF_VR训练好的模型在所有数据集上的性能
...
# 参数设置
MODEL_CKPT_PATH="${1:-ckpt/exp_lwf_vr_fair}"
GPU="${2:-4,5,6,7}"
TEST_MODE="${3:-whole}"
...
for model_dataset in "${datasets[@]}"; do
    ...
    for test_dataset in "${datasets[@]}"; do
        ...
        # 运行LWF_VR模型评估 - 使用专门的测试脚本，只使用第一个GPU避免并行问题
        FIRST_GPU=$(echo $GPU | cut -d',' -f1)
        result=$(CUDA_VISIBLE_DEVICES=$FIRST_GPU python test_lwf_model.py \
