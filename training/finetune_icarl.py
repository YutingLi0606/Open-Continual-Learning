...
from training.finetune_zscl import finetune as pure_finetune_fewshot
...
def finetune(args, ref_images):
    ...
    # prepare dataset
    dataset_class = getattr(dataloader, args.train_dataset)
    ...
    if args.few_shot > 0:
        print('=====few-shot======')
        print(f'Using {args.few_shot}-shot training')
        # create few_shot data
        few_shot_data = {}
        ...
        print(f"Few-shot data loader length: {len(few_shot_data_loader)}")
...
class DynamicDataset():
    ...
    def constructExampleSet(self, args):
        ...
        # Few-shot data processing for iCaRL constructExampleSet
        if args.few_shot > 0:
            print('=====few-shot for iCaRL constructExampleSet======')
            print(f'Using {args.few_shot}-shot for example set construction')
            new_dataset = self._getFewShotDataset(args)
        else:
    ...
    def _getFewShotDataset(self, args):
        """Get few-shot dataset for iCaRL example set construction"""
        dataset_class = getattr(dataloader, self.cur_dataset)
        ...
        for sample in tqdm(update_dataset.train_dataset):
            label = sample[1]  # assuming label is at index 1
            if label not in few_shot_data:
                few_shot_data[label] = []
            if len(few_shot_data[label]) < args.few_shot:
                few_shot_data[label].append(sample[0].tolist())  # assuming image is at index 0
        
        # Flatten the few-shot data
        samples = []
        for label, images in few_shot_data.items():
