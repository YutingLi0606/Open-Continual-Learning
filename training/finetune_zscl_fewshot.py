...
from methods.zscl.helpers import get_datasets_text, merge_we, wise_we, moving_avg, l2_loss, virtual_vocab, distillation
...
def finetune(args):
    ...
    if args.we_wise or (args.wise_merge and args.wise_ft_model != "zeroshot"):
        ...
        model_fix, train_preprocess, val_preprocess = clip.load(args.model, args.device, jit=False)
        if args.load is not None:
    ...
    if args.method == "ZSCL":
        ...
        ref_dataset_cls = getattr(dataloader, args.ref_dataset)
        ...
        if args.text_datasets is not None:
            ...
            ref_sentences_cls = getattr(dataloader, args.ref_sentences)
