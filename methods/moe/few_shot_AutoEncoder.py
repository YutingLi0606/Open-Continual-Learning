...
def few_shot_AutoEncoder(args):
    if args.eval_only == True:
        ...
        _, _, val_preprocess = clip.load(args.model, jit=False)
        ...
        for j, dataset_name in enumerate(args.eval_datasets):
            ...
            dataset_class = getattr(dataloader_module, args.eval_datasets[j])
        ...
        _, train_preprocess, val_preprocess = clip.load(args.model, jit=False)  # model='ViT-B/16'
        Autoencoder_list = nn.ModuleList()
        for i in range(12):
            model_autoencoder = few_shot_autoencoder(256 * 13 * 13).to(device)
            Autoencoder_list.append(model_autoencoder)
        ...
        dataset_class = getattr(dataloader_module, args.train_dataset)
        dataset = dataset_class(  #
            train_preprocess,
            location=args.data_location,
            batch_size=args.batch_size,
